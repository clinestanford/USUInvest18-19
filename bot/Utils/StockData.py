...
class StockData:
    ...
    def CandleToDataFrame(df, stock, candle):
        temp = chart(candle['date'], stock, candle['open'], candle['high'], candle['low'], candle['close'], candle['volume'], candle['change'], candle['changePercent'], candle['vwap'])
    ...
    def Download(symbols = "SPY,APPL,F,GE,FB"):
        ...
        for stock in stockData:
            try:
                df = pd.read_pickle("StockData/" + stock + ".pkl")
            except IOError:
                df = pd.DataFrame(columns=columns)

            #If we don't have a pickle, get the 5 year data
            if (not os.path.exists("StockData/" + stock + ".pkl")):
